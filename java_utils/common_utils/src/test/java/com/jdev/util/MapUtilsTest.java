...
class MapUtilsTest {
    ...
    void test_createAndFillMap() {
        ...
        expected.put("Spartak", 1950);
        expected.put("Barcelona", 1983);
        Map<String, Integer> actual = MapUtils.createAndFillMap(new String[]{"Spartak", "Barcelona"}, new Integer[]{1950, 1983});
        Assertions.assertEquals(expected, actual);
    }

    @Test
    void test_createAndFillMap_throwException_2arraysNotTheSameSize() {
        HelperException helperException = Assertions.assertThrows(HelperException.class, () -> MapUtils.createAndFillMap(new String[]{"Spartak"},
    ...
    void test_createAndFillMap_withPair() {
        ...
        expected.put("Spartak", 1950);
        expected.put("Barcelona", 1983);
        Map<String, Integer> actual = MapUtils.createAndFillMap(new MapUtils.PairForMap[]{new MapUtils.PairForMap<>("Spartak", 1950),
    ...
    void test_createAndFillMap_withList(){
        Map<Integer, Double> actual = MapUtils.createAndFillMap(List.of(MapUtils.PairForMap.createKvPairForMap(100, 50.292),
                MapUtils.PairForMap.createKvPairForMap(200, 25.190)));
